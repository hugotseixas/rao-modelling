...
format: 
  html:
    ...
    code-fold: true
    code-tools: true
    source: https://github.com/hugotseixas/rao_modelling/blob/main/lulc_eco_nunes/lulc_eco.qmd
    self-contained: true
...
I will try different models to perform the predictions and classifications, and at the same time, will try to present the data and results in a intuitive manner.
...
train_data <- filter(data, region == "STM")
test_data  <- filter(data, region == "PGM")
...
biod_fold <- vfold_cv(train_data, v = 10)
...
lulc_fold <- bootstraps(train_data, times = 10)
...
lulc_mod <-
  decision_tree(
    cost_complexity = lulc_best$cost_complexity,
    tree_depth = lulc_best$tree_depth,
    min_n = lulc_best$min_n
  ) %>%
  set_engine("rpart") %>%
  set_mode("classification")

lulc_wflow <- lulc_wflow %>%
  update_model(lulc_mod)

lulc_fit <- 
  lulc_wflow %>%
  fit(data = train_data)

lulc_aug <- augment(lulc_fit, test_data)
...
mod_met <- lulc_aug %>% 
  roc_auc(truth = lu_uf, .pred_UF:.pred_MA) %>%
  mutate(.estimate = round(.estimate, digits = 2))
...
lulc_best <- lulc_res %>% 
  select_best(metric = c("roc_auc"))

lulc_mod <-
  rand_forest(
    mtry = lulc_best$mtry,
    trees = lulc_best$trees,
    min_n = lulc_best$min_n
  ) %>%
  set_engine("ranger", importance = "impurity") %>%
  set_mode("classification")
