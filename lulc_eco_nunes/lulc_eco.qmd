...
muni <- 
  read_municipality(code_muni = "PA", showProgress = FALSE) %>% 
    ...
    .f = function(r, z) {
      ...
      data <-
        read_parquet(
          ...
          col_select = c(
            "region", "transect_code", "land_useclass", "utm_x", "utm_y"
          )
        ) %>%
        filter(region == r) %>%
        distinct(region, transect_code, land_useclass, .keep_all = TRUE) %>%
        mutate(
          land_useclass = fct_relevel(
            factor(land_useclass),
            c(
              "Primary forest", "Secondary forest", "Reforestation",
              "Small holder agriculture", "Mechanized agriculture",
              "Fruticulture", "Abandoned plantation", "Pasture"
            )
          )
        ) %>%
        st_as_sf(
    ...
    .f = function(r, m) {
      ...
      ggplot() +
        ...
        geom_sf_interactive(
          ...
          aes(
            color = land_useclass,
            tooltip = land_useclass,
            data_id = land_useclass,
            `data-id` = land_useclass
          ),
          size = 1,
        ...
        scale_color_manual_interactive(
          ...
          extra_interactive_params = "data-id",
          `data-id` = data %>% 
            arrange(data, land_useclass) %>% 
            distinct(land_useclass),
        ...
        guides(
          color = guide_legend_interactive(
            ncol = 1,
...
girafe(
  code = 
    print(
      (plot[[2]] + theme(legend.position = "")) /
        plot[[1]] + plot_layout(guides = "collect")
      ), 
  options = list(
    opts_tooltip(
...
data <- read_parquet(
  ...
  summarise(richness = sum(number, na.rm = TRUE), .groups = "drop_last") %>%
  mutate(
    id = cur_group_id(),
    label = glue(
      "Total Observations: {n()}",
      "\n",
      "Mean: {round(mean(richness))}",
      "\n",
      "Median: {round(median(richness))}",
      "\n",
      "Variance: {round(var(richness))}"
    )
  ) %>%
  ungroup() %>%
...
read_parquet(
    file = "data/lulc_eco.parquet",
    col_select = c(
      "region", "transect_code", "land_useclass", "ph":"al"
    )
  )
